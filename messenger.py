            ...
            if data.startswith("[NAME_LIST]"):
                ...
                if chat_log_length > 0:
                    for i in range(int(chat_log_length)):
                        ...
                        lines = [
                            message[1].lstrip()[i : i + wraplength]
                            for i in range(0, len(message[1].lstrip()), wraplength)
                        ]
                        display_messages = CTkButton(
                ...
                display_messages = CTkButton(
                    ...
                    + " : {}\n{}".format(
                        lines[0],
                        indent("\n".join(lines[1:]), " " * (len(message_header) + 2)),
                    ),
                    font=better_font,
                    anchor="nw",
                    fg_color=message_color,
...
def send(message, message_panel, username, entry):
    ...
    if message != "":
        ...
        lines = [
            message[i : i + wraplength] for i in range(0, len(message), wraplength)
        ]
        display_message = CTkButton(
            master=message_panel,
...
def send_messages(send_message_panel, message_panel, username):
    ...
    entry.bind(
        "<Return>", lambda event: send(entry.get(), message_panel, username, entry)
    )
...
def main_gui(username):
    ...
    message_panel = CTkScrollableFrame(
        master=root, border_color=borders, border_width=borders_width
    )
    message_panel.grid(row=0, column=1, sticky="nsew", padx=20, pady=20)

    slider_panel = CTkScrollableFrame(
        master=root,
        border_color=borders,
        border_width=borders_width,
        label_text="Online Users : ",
    ...
    send_message_panel = CTkFrame(
        master=root, height=10, border_color=borders, border_width=borders_width
    )
    send_message_panel.grid(
    ...
    Thread(
        target=send_messages, args=(send_message_panel, message_panel, username)
    ).start()
...
def startup():
    ...
    button = CTkButton(
        ...
        corner_radius=20,
        font=better_font,
        command=lambda: initiate_main_gui(entry.get()),
    )
